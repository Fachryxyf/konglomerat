...
// Static visual config per difficulty; labels/descriptions are resolved via i18n.
const DIFFICULTY_META: Record<AIDifficulty, { key: string; Icon: LucideIcon; color: string }> = {
  EASY: { key: "easy", Icon: Smile, color: "text-emerald-500" },
  MEDIUM: { key: "medium", Icon: Target, color: "text-amber-500" },
  HARD: { key: "hard", Icon: Flame, color: "text-rose-500" },
};
...
export default function PlayerSetup() {
  ...
  const [players, setPlayers] = useState<PlayerConfig[]>(
    Array(4).fill(null).map((_, i) => ({
      name: t("ui.setup.defaultName", { n: i + 1 }),
      type: i === 0 ? "HUMAN" : "AI",
  ...
  return (
    <div className="h-screen flex items-center justify-center p-3 sm:p-4 bg-gradient-to-br from-emerald-900 via-emerald-800 to-teal-900">
      <Card className="w-full max-w-4xl h-[min(94vh,780px)] p-0 gap-0 shadow-2xl flex flex-col overflow-hidden">
        {/* Header */}
        <div className="relative text-center px-6 pt-5 pb-3 shrink-0 border-b border-zinc-200 dark:border-zinc-800">
          <LanguageToggle className="absolute top-4 right-4 text-zinc-600 dark:text-zinc-300" />
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">{t("ui.setup.subtitle")}</p>
        </div>
        ...
        <div className="flex-1 min-h-0 grid md:grid-cols-2 gap-4 px-6 py-4 overflow-y-auto md:overflow-hidden scrollbar-thin">
          ...
          <div className="flex flex-col min-h-0">
            <div className="grid grid-cols-2 gap-3 shrink-0">
              <div>
                <Label className="text-sm font-semibold">{t("ui.setup.playerCount")}</Label>
                <Select value={String(playerCount)} onValueChange={(v) => updatePlayerCount(Number(v))}>
                  <SelectTrigger className="mt-1 h-9"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {[2, 3, 4, 5, 6, 7, 8].map((n) => (
                      <SelectItem key={n} value={String(n)}>{t("ui.setup.nPlayers", { n })}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              <div>
                <Label className="text-sm font-semibold">{t("ui.setup.startingCash")}</Label>
            ...
            <Label className="text-sm font-semibold mt-3 mb-1.5 shrink-0">{t("ui.setup.playersLabel")}</Label>
                ...
                <div key={i} className="rounded-lg border bg-card hover:bg-accent/50 transition p-2.5">
                  <div className="flex items-center gap-2.5">
                    ...
                    <Input
                      ...
                      placeholder={t("ui.setup.namePlaceholder", { n: i + 1 })}
                      className="flex-1 h-9"
                    />
                    <Select value={p.type} onValueChange={(v) => updatePlayer(i, "type", v)}>
                      <SelectTrigger className="w-28 h-9"><SelectValue /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="HUMAN"><span className="inline-flex items-center gap-1.5"><User className="w-3.5 h-3.5" /> {t("ui.setup.human")}</span></SelectItem>
                        <SelectItem value="AI"><span className="inline-flex items-center gap-1.5"><Bot className="w-3.5 h-3.5" /> {t("ui.setup.ai")}</span></SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  {p.type === "AI" && (
                    <div className="mt-2">
                      <div className="grid grid-cols-3 gap-1">
                        {(["EASY", "MEDIUM", "HARD"] as AIDifficulty[]).map((diff) => {
                          const info = DIFFICULTY_META[diff];
          ...
          <div className="md:overflow-y-auto scrollbar-thin space-y-3 md:pr-1">
            <div className="rounded-lg bg-emerald-50 dark:bg-emerald-950/30 p-3 text-sm text-muted-foreground border border-emerald-200 dark:border-emerald-900">
              <p className="font-semibold text-emerald-700 dark:text-emerald-400 mb-1 flex items-center gap-1.5"><ClipboardList className="w-4 h-4" /> {t("ui.setup.rulesTitle")}</p>
              <ul className="space-y-1 text-xs">
                <li>• {t("ui.setup.rule.cash", { cash: startingCash.toLocaleString() })}</li>
                <li>• {t("ui.setup.rule.go")}</li>
                <li>• {t("ui.setup.rule.jail")}</li>
                <li>• {t("ui.setup.rule.build")}</li>
                <li>• {t("ui.setup.rule.auction")}</li>
                <li>• {t("ui.setup.rule.supply")}</li>
              </ul>
            </div>
            <div className="rounded-lg bg-blue-50 dark:bg-blue-950/30 p-3 text-xs border border-blue-200 dark:border-blue-900">
              <p className="font-semibold text-blue-700 dark:text-blue-400 mb-1.5 flex items-center gap-1.5"><Bot className="w-4 h-4" /> {t("ui.setup.aiLevelTitle")}</p>
              <div className="space-y-1 text-[11px]">
                {(Object.keys(DIFFICULTY_META) as AIDifficulty[]).map((diff) => {
                  const info = DIFFICULTY_META[diff];
                  return (
                    <div key={diff} className="flex items-start gap-1.5">
                      <info.Icon className={"shrink-0 w-3.5 h-3.5 mt-0.5 " + info.color} />
                      <div>
                        <span className="font-semibold">{t(`ui.ai.${info.key}.label`)}:</span>{" "}
                        <span className="text-muted-foreground">{t(`ui.ai.${info.key}.desc`)}</span>
                      </div>
                    </div>
                  );
                })}
        ...
        <div className="px-6 py-3 shrink-0 border-t border-zinc-200 dark:border-zinc-800">
          <Button onClick={startGame} size="lg" className="w-full text-base font-semibold bg-emerald-600 hover:bg-emerald-700 gap-2">
            <Play className="w-5 h-5 fill-current" /> {t("ui.setup.start")}
          </Button>
        </div>
      </Card>
    </div>
