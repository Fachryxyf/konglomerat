...
- [x] **Migrasi call-site UI → `dispatch` SELESAI.** Tidak ada lagi komponen atau hook yang memanggil action aksi-pemain langsung; semuanya lewat `useIntent()` (dalam render) atau `sendIntent()` (timer & AI) di `use-intent.ts`. Efeknya: setiap perubahan state pemain kini melewati 3 gerbang yang sama yang akan dijalankan server, dan penolakan tampil sebagai toast alih-alih gagal diam-diam.
- [x] **Aktor eksplisit di setiap call-site.** Dulu `dispatch` jatuh ke `currentPlayerIndex` sebagai aktor; itu salah untuk lelang (penawar bukan pemilik giliran), fiskal & rescue (antrean keputusan), serta terima/tolak trade (penerima). Semua sekarang mengirim `actorId` eksplisit — persis kontrak yang dipakai server (aktor dari sesi, bukan dari payload).
- [x] **`AUCTION_LEAVE` masuk kosakata `Intent`** (+ skema zod + case validator, digabung dengan `AUCTION_PASS` karena secara aturan identik).
...
- [ ] **Aksi non-intent yang masih langsung** (bukan aksi pemain, jadi di luar cakupan gerbang): `init`/`reset` (siklus hidup match — nanti jadi wewenang server/lobby), `clearEvent`/`dismissRent` (murni penutup UI), dan `endAuction` (resolusi milik engine sendiri, bukan permintaan pemain).
